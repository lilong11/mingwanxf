...
function imageUploader(obj, more,curid) {
    $.ajax({
        type: "get",
        url: URL + "hzapp/file/index",
        data: {},
...
function removeImage(obj, more,curid='albumsClone') {
    var parentObj = $(obj).parent().parent();
    parentObj.find('input').val('');
    parentObj.find('img').attr('src', PUBLIC +'/static/dist/img/noimage.gif');

    var length = $('#'+curid).find('.fileupload-new.img-thumbnail').length;
    if (more && length > 1) {
        parentObj.remove();
    }
}
